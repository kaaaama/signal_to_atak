...
.PHONY: help up down restart build ps logs logs-bot shell lint test test-fast licenses \
...
help:
	@printf "%s\n" \
		...
		"make test                   Rebuild the bot image and run pytest in Docker" \
		"make test-fast              Run pytest in the current bot image" \
